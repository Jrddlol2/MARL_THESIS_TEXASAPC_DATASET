...
Simulation horizon & --- & Single observed Route 801 service window (\texttt{\%TODO-DATA}: select start/end after historical schedule and timestamp-basis validation) & Section~\ref{sec:research-design} \\
...
Control stop count & --- & \texttt{\%TODO-VAL}: select from 29 observed stop IDs using Section~\ref{subsec:control-stop-selection} criteria after calibration/validation & Section~\ref{subsec:control-stop-selection} \\
...
Maximum holding duration & $\Delta T$ & \%TODO-VAL: to be set during implementation & Section~\ref{subsec:marl-arch} (Action Space) \\
...
Discount / event-based discount & $\gamma$, $\beta$ & \%TODO-VAL: tuned during implementation & Section~\ref{subsec:marl-arch} \\
...
Demand-surge scale & $f_d$ & $\mathcal{N}(1,\sigma_d^2)$ with \%TODO-VAL $\sigma_d$ and support clipped to $[1,3]$ & Section~\ref{subsec:stochastic-vars} \\
Traffic-speed stress scale & $f_s$ & $\mathcal{N}(1,\sigma_s^2)$ with \%TODO-VAL $\sigma_s$ and support clipped to $[0.8,1.2]$ & Section~\ref{subsec:stochastic-vars} \\
Breakdown rate & $\lambda$ & \texttt{\%TODO-VAL}: declared scenario rate (events per bus-hour); APC contains no failure events & Section~\ref{subsec:stochastic-vars} \\
