...
This chapter describes the development, training, and evaluation of a Multi-Agent Reinforcement Learning (MARL) model for dynamic bus scheduling under non-ideal conditions. The goal is a reproducible pipeline from the public-data-calibrated CapMetro Route 801 direction-code 6 case---introduced in Section~\ref{sec:background} and selected as the data-richer corridor in Section~\ref{sec:rationale}---to measurable controller performance under baseline and disturbed operating conditions.


\section{Research Design}\label{sec:research-design}

The study uses a \textbf{controlled comparative experimental design} with control strategy (No Control, Forward Headway, Even Headway, MARL) and the activation-matrix condition as factors. Conditions include the D+T Stage A baseline, the D+T+S+W+B combined evaluation, the S/W/B single-disturbance ablations, and the separate synthetic $\eta$ sweep (here D~=~baseline demand, S~=~demand surge, T~=~ordinary travel-time variation, W~=~weather stress, and B~=~breakdown; these five classes are defined in Section~\ref{subsec:stochastic-vars}, and their on/off status in each condition is given in the activation matrix, Table~\ref{tab:activation-matrix}). The response variables are simulated mean passenger waiting time, mean bus travel time, and headway coefficient of variation.
...
A pure-Python alternative without SUMO calibration was also considered but rejected: it would lack the empirically grounded per-segment travel-time distributions that the calibrated SUMO model provides. Retaining SUMO for training as well keeps one environment for calibration, baselines, training, and evaluation; the measured cost of a simulated service day makes that affordable on accessible hardware, and the learner itself is small enough to train on CPU. Rodriguez et al.~\cite{Rodriguez2023Cooperative} and Wang and Sun~\cite{Wangsun} adopt the same two-layer approach, using event-based custom simulators with calibration parameters supplied from empirical data.



As shown in Figure~\ref{fig:pipeline}, the pipeline proceeds in two phases. First, the per-segment travel time, demand, and dwell time distributions are estimated from the APC records and the corridor network is calibrated to them. Second, every experiment---the three baselines, training, and Monte Carlo evaluation---runs that one calibrated corridor, so all controllers face an identical environment by construction and no cross-simulator equivalence argument is required. Rodriguez et al.~\cite{Rodriguez2023Cooperative} and Wang and Sun~\cite{Wangsun} instead train in event-based mesoscopic simulators parameterized by empirical corridor data; that is the cheaper design, and it remains the fallback if the episode budget grows beyond what the microscopic loop can deliver.
...
Before MARL training begins, the SUMO layer is verified against held-out CapMetro observations. GEH and RMSPE are both applied to per-segment running time, computed as \texttt{rev\_seconds} minus the current-stop \texttt{dwell\_time}: because \texttt{rev\_seconds} is recorded open-to-open it already includes that stop's dwell, which is modelled separately, so the two are never double-counted. Average speed may be added only after the unit of \texttt{rev\_distance} is confirmed from authoritative odometer metadata. Event coordinates support location-quality checks but are not described as continuous trajectories.
...
\begin{figure}[htbp]
    ...
    SUMO calibration output. (a) GEH statistic for observed and simulated
    per-segment running times. (b) Simulated segment running time against
    held-out empirical \texttt{rev\_seconds} observations in the RMSPE
...
The GEH statistic, introduced for traffic-modelling acceptance testing in the UK Design Manual for Roads and Bridges \cite{DMRBV12}, is a relative-difference measure that tolerates larger absolute errors on larger quantities. It is defined on hourly traffic volumes; here it is applied to per-segment running times as a closeness statistic, with RMSPE as the binding criterion, because bus volume in the simulator is set by the dispatch schedule and would be matched trivially. This use is stated explicitly rather than implied, and is illustrated in panel (a) of Figure~\ref{fig:calibration-illustrative}:

\begin{equation} GEH = \sqrt{\frac{2(M - C)^2}{M + C}} \label{eq:geh} \end{equation}

where $M$ is the simulated per-segment running time and $C$ is its observed counterpart. The environment is accepted when $GEH < 5$ for more than 85\% of corridor segments, following the FHWA Traffic Analysis Toolbox calibration criterion~\cite{FHWA2019TAT} as applied in recent SUMO calibration work by Patil et al.~\cite{Patil2025Conformal}.

\subsubsection{Segment Travel-Time Calibration via RMSPE}

The binding calibration criterion is the root mean squared percentage error, which weights every segment by its own magnitude and is therefore comparable across the short downtown segments and the long suburban ones, illustrated in panel (b) of Figure~\ref{fig:calibration-illustrative}:

\begin{equation} RMSPE = \sqrt{\frac{1}{n}\sum_{i=1}^{n}\left(\frac{x_{obs} - x_{model}}{x_{obs}}\right)^{2}} \label{eq:rmspe} \end{equation}

where $x_{obs}$ is an observed segment running time and $x_{model}$ is its simulated counterpart. Edge speeds are tuned on the calibration service days only, then evaluated once on the held-out service days. A speed-based secondary error measure is permitted only after distance units are verified. Until then, the travel-time measure is the reproducible calibration target.
...
\textbf{Training condition.} The policy is trained with D+T always active while S, W, and B are domain-randomized over their declared ranges: in each episode surge, weather, and breakdown are independently active with probability $0.5$, and whenever weather is active its synthetic intensity is drawn as $\eta \sim \mathrm{Uniform}(0, 1.3)$, where $\eta = 0$ leaves only the observed ordinary-rain multiplier. Weather exposure therefore includes observed ordinary-rain strata where empirical support is adequate and labeled synthetic lognormal stress outside that support, and the evaluation intensities all lie inside the training range.
...
Simulation horizon & --- & One weekday service window, 07:00--18:00, the band the archived timetable runs at a 10-minute headway; 18 trips are dispatched into it per episode & Section~\ref{sec:research-design}, archived timetable~\cite{CapMetro2021Timetable801} \\
...
Maximum holding duration & $\Delta T$ & 300\,s, so the largest hold is $0.4\,\Delta T = 120$\,s, the absolute cap used in the reviewed holding studies~\cite{Rodriguez2023Cooperative,Tang2024Curriculum}; $\Delta T = H_0 = 600$\,s (240\,s cap) is a sensitivity case & Section~\ref{subsec:marl-arch} (Action Space) \\
Holding action bins & $\Omega$ & $\{0.0, 0.1, 0.2, 0.3, 0.4\}$ & Section~\ref{subsec:marl-arch} (Action Space) \\
Action space size per agent & $|A_i|$ & 10 (5 hold $\times$ 2 skip) & Section~\ref{subsec:marl-arch} (Action Space) \\
Monte Carlo runs per cell & $N_{\text{runs}}$ & $\geq 30$ & Section~\ref{sec:research-design} \\
...
Synthetic severe-weather intensity & $\eta$ & $\{0.0, 0.3, 0.6, 1.0, 1.3\}$; values above observed support are stress tests & Section~\ref{subsec:stochastic-vars} \\
Demand-surge scale & $f_d$ & $\mathcal{N}(1,\sigma_d^2)$ clipped to $[1,10]$, with $\sigma_d = 1$ (sensitivity: 2, 3), following Wang and Sun~\cite{Wangsun} & Section~\ref{subsec:stochastic-vars} \\
Traffic-speed stress scale & $f_s$ & $\mathcal{N}(1,\sigma_s^2)$ clipped to $[0.8,1.2]$~\cite{Wangsun}; $\sigma_s = 0$ in the main evaluation because ordinary variation is already the fitted empirical distribution (T), and $\sigma_s \le 0.3$ is available as a stress case & Section~\ref{subsec:stochastic-vars} \\
Breakdowns per episode & $n_B$ & 1 (sensitivity case: 3), following the single- and multiple-disruption designs of Guedes and Borenstein~\cite{Guedes2018Rescheduling}; APC contains no failure events & Section~\ref{subsec:stochastic-vars} \\
...
Table~\ref{tab:sim-parameters} is the parameter reference for this chapter. Values marked ``to be finalized'' are intentionally deferred at this proposal stage and will be set during the implementation phase: those noted \emph{data pending} await a verified external source or an unfinished empirical derivation, while those noted \emph{design} are experimental choices, several fixed through the Expected~Output~2.1 sensitivity analysis. Marking them explicitly keeps unsupported target values from being mistaken for observed CapMetro quantities. The 29-stop count is reproduced from the clean direction-code 6 subset. The demand and speed clip supports are not standard deviations: $\sigma_d$ and $\sigma_s$ remain separate distribution parameters and cannot be replaced by their clip bounds.
...
\item \textbf{Historical schedule and fleet metadata.} A 2021-compatible GTFS snapshot would be the authoritative source for direction labels, stop names, route shape, and scheduled headway, and a verified fleet source for capacity. No such snapshot is publicly archived: six retrieval routes were exhausted and the gate is therefore closed as unavailable rather than pending. Two parameters it blocked are sourced elsewhere and labelled accordingly---the scheduled headway from the archived 2021 timetable~\cite{CapMetro2021Timetable801}, and the direction label as corroboration from the current feed~\cite{CapMetroGTFS}, which matches 28 of the 29 study stop IDs in order at a median offset of 8.5\,m. Historical stop names are not claimed, and vehicle capacity remains to be finalized (data pending).
\end{enumerate}

Table~\ref{tab:field-mapping-texas} summarizes how each verified CapMetro and NOAA field maps to the quantity derived from it and the role, and limitation, that quantity carries in the model.
...
\texttt{rev\_seconds}, \texttt{rev\_distance} & Per-segment running time (\texttt{rev\_seconds} is open-to-open, so the current-stop dwell is netted out) and possible average speed & Travel-time RMSPE is primary; distance unit remains unresolved \\
...
SUMO speed calibration (RMSPE, Section~\ref{subsec:stochastic-vars}); anchors the traffic-speed generator ($\sigma_s$) and the weather generator's baseline mean ($\mu$) \\
...
\item \textbf{Discrete bus breakdown (B):} a discrete event that permanently removes a bus from the active agent set for the remainder of the episode; $n_B$ buses fail per episode (Table~\ref{tab:notation}).
\end{itemize}
...
$n_B$ &
Number of buses that fail in an episode, each at a randomly drawn bus and stop. $n_B=0$ disables breakdowns. &
buses per episode \\
...
The synthetic factor is swept over $\eta \in \{0.0,0.3,0.6,1.0,1.3\}$. Values through 1.0 span the range evaluated by Patil et al.; 1.3 is a deliberate extrapolation. None is assigned a rain-rate label. Observed ordinary-rain experiments are reported by their NOAA exposure definition rather than by $\eta$. Table~\ref{tab:eta-basis} summarizes the basis for each swept intensity value.
...
Unlike the perturbations above, a bus breakdown is a \textit{discrete event} that removes a vehicle from active simulation. Guedes and Borenstein~\cite{Guedes2018Rescheduling} model serious public-transit vehicle failures in which the disabled vehicle is towed and its trip is cut, and Daganzo~\cite{Daganzo2009} describes a failed bus going out of service with its run reassigned to the following bus; these two sources support the removal semantics. Cao et al.~\cite{Cao2022Train} model train malfunctions in an adjacent MARL setting as temporary stops of random duration triggered by a Poisson process, so they support random event timing but not removal. None of these sources provides an empirical CapMetro failure rate; the APC data contain no breakdown field. Breakdowns are therefore a labeled synthetic stressor. The count follows the experimental design of Guedes and Borenstein~\cite{Guedes2018Rescheduling}, who introduce one disruption and then two to three near-simultaneous disruptions as a robustness test: each episode removes $n_B = 1$ bus in the main evaluation and $n_B = 3$ buses in a sensitivity case (Table~\ref{tab:notation}). The failed bus and the stop at which it fails are drawn from the episode seed before the simulation starts, so every controller faces the identical breakdown. The failed bus is removed for the remainder of the episode; passengers on board alight at that stop and wait for the next bus to their own destination. When a breakdown occurs at bus $b_k$, $b_k$ is removed from the active agent set, leaving an enlarged headway between the trailing bus $b_{k-1}$ and the leading bus $b_{k+1}$. The passengers that $b_k$ would have served accumulate at the stops ahead of $b_{k-1}$, so the trailing bus inherits this residual demand as it advances.
...
\subsection{MARL Architecture and MDP Formulation}\label{subsec:marl-arch}
...
\begin{itemize}
\item \textbf{Holding control:} a holding-strength parameter $\alpha_{i,t} \in \Omega = \{0.0, 0.1, 0.2, 0.3, 0.4\}$, multiplied by a maximum holding duration $\Delta T = 300$\,s to yield seconds held, so holds are 0, 30, 60, 90 or 120\,s; $\alpha_{i,t} = 0$ means no holding.
...
where $w_1$, $w_2$, and $w_3$ denote the weighting coefficients to be determined through the Expected Output 2.1 sensitivity analysis. Each component is formulated as a non-positive penalty, allowing the agent to maximize its cumulative return in Eq.~\eqref{eq:bellman} by minimizing headway irregularity, passenger waiting time, and unnecessary stop-skipping behavior. Consequently, this chapter establishes the reward formulation and its optimization objective, while the specific mathematical expressions and coefficient values are reserved for the implementation and evaluation phase.
...
\begin{figure}[htbp]
    ...
    cycle during training. Authors' illustration, following the agent--environment cycle formalism of Terry et al.~\cite{Terry2021PettingZoo}.}
    \label{fig:aec-training}
\end{figure}
 

Because heavy-tailed returns can destabilize value-based learning if injected without safeguards \cite{Henderson2018Matters,Agarwal2021Precipice}, training under disturbance is paired with the stabilizers discussed in Section~\ref{sec:challenges}: DDQN's double estimator, reward clipping (per-transition rewards are clipped at $-5$, a bound chosen from the observed reward distribution under the most severe weather intensity, where 95\% of rewards exceed $-4.4$), and slow target-network updates (Polyak averaging with $\tau = 0.005$ after every gradient step). If training under the full disturbance set proves unstable at the highest intensities despite these measures, a curriculum that ramps disturbance intensity over the course of training~\cite{Tang2024Curriculum} is retained as a fallback.
...
\begin{equation}
d_{FH} = \max\!\left\{0,\, \min\!\left[\bar{d} + g\,(H_0 - h^-),\; h_{\max}\right]\right\}
\label{eq:fh}
\end{equation}

where $H_0$ is the desired (scheduled) headway, $h^-$ is the forward headway (time gap to the bus ahead), $\bar{d}$ is the average slack delay at equilibrium, and $g > 0$ is a control gain parameter. If a bus is running closer to the bus ahead than scheduled ($h^- < H_0$), it holds for an additional period proportional to the gap deficit; if it is already at or beyond the desired spacing ($h^- \geq H_0$), it does not hold. Following Daganzo, $g = \alpha + b_s$, where $b_s$ is the extra bus delay per second of headway (the boarding rate over the stops up to the next control stop times the fitted seconds per boarding, Daganzo's Eq.~3) and $\alpha = 0.2$; the slack is $\bar{d} = 25$\,s, the values of Daganzo's worked example. The hold is capped at the same $h_{\max} = 120$\,s as EH and MARL. FH represents local, single-agent reactive control: each bus reacts only to information about the bus immediately ahead. Because Eq.~\eqref{eq:fh} conditions only on $h^-$, FH is expected to partially correct demand- and traffic-induced bunching but to respond poorly to a breakdown: it has no observation of the enlarged gap a failed bus leaves for the bus behind it, so the trailing bus's holding decision is uninformed by the breakdown until the irregularity has already propagated back to it.
...
\begin{equation}
T_{EH} = \max\!\left\{\min\!\left[\frac{\hat{h}^+ - h^-}{2},\; h_{\max}\right],\, 0\right\}
\label{eq:eh}
\end{equation}

where $h^-$ is the forward headway, $\hat{h}^+$ is the estimated backward headway (time gap to the bus behind), and $\frac{\hat{h}^+ - h^-}{2}$ is the holding time that would split the difference evenly between the two gaps. The hold is capped at $h_{\max} = 120$\,s. Rodriguez et al.\ bound holds at $0.4\,H$ to prevent excessive intervention, which at their 5-minute headway is 120\,s, and note that the value should follow agency policy; the other holding studies reviewed cap holds at 90--120\,s~\cite{Wang2023MultiObj,Liu2023DRLHolding,Tang2024Curriculum}. Applying $0.4\,H_0$ at this corridor's 10-minute headway would allow 240\,s, beyond any cap in the reviewed studies, so 240\,s is reported only as a sensitivity case. The cap equals the largest MARL hold $0.4\,\Delta T$, so EH and MARL are constrained to the same maximum hold and the comparison is fair. EH represents globally-aware reactive control: it uses information about both neighbors but follows a fixed rule rather than a learned policy. Because Eq.~\eqref{eq:eh} is a fixed linear rule computed from the currently observed gaps, EH is expected to handle breakdowns better than FH, since the enlarged backward gap $\hat{h}^+$ enters the rule directly, but it has no mechanism to anticipate or discount the heavier-tailed delays that the weather generator introduces, since the rule was not designed with a heavy-tailed disturbance in mind.
...
Figure~\ref{fig:stage-a-illustrative} shows the format in which these Stage~A comparisons are reported across the four controllers and three response variables.
...
\paragraph{Stage B acceptance criterion} The MARL policy is reported as performing well under disturbance if its mean waiting time stays below that of the best-performing baseline across the full sweep, with statistical significance under the same correction as Stage A. Figure~\ref{fig:aec-eval} illustrates the matched-seed Monte Carlo evaluation procedure used for these cells.
