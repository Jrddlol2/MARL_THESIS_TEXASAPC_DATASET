...
#
# Format: every entry has a clearly separated BEFORE block and AFTER block
# (blockquoted, so they're visually boxed off from each other and from the
# surrounding text) — skim the labels, not the paragraphs, to see what
# changed. Within each block, the part that's actually new/different is
# **bolded**; unchanged surrounding text stays plain so your eye goes
